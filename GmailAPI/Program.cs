    ...
    class Program
    {
                ...
                if (ListResponse != null && ListResponse.Messages != null)
                {
                    ...
                    foreach (Message Msg in ListResponse.Messages)
                    {
                        ...
                        if (MsgContent != null)
                        {
                            ...
                            string outputDir = Directory.GetParent(Directory.GetParent(Directory.GetCurrentDirectory()).ToString()) + Convert.ToString(ConfigurationManager.AppSettings["GmailAttach"]);
                            List<string> FileName = GmailAPIHelper.GetAttachments(HostEmailAddress, Msg.Id, outputDir);
                            ...
                            if (!string.IsNullOrEmpty(ReadableText))
                            {
                                ...
                                if (!string.IsNullOrEmpty(Date))
                                {
                                    ...
                                    DateTime date = ConvertToDateTime(Date);
                                    GMail.MailDateTime = date;
                                    Console.WriteLine(date);
                                    Console.WriteLine(GMail.Body);
                                }
        ...
        private static DateTime ConvertToDateTime(string strDateTime)
        {
            ...
            catch (Exception e)
            {
                ...
                string Minutes;
                string seconds;
                strDateTime = strDateTime.Substring(5);
                if (strDateTime.Substring(1, 1) != " ")
                {
                    //Console.WriteLine("first way");
                    Day = strDateTime.Substring(0, 2);
                    Month = strDateTime.Substring(3, 3);
                    Year = strDateTime.Substring(7, 4);
                    Hour = strDateTime.Substring(12, 2);
                    Minutes = strDateTime.Substring(15, 2);
                    seconds = strDateTime.Substring(18, 2);
                }
                else
                {
                    //Console.WriteLine("second way");
                    Day = "0" + strDateTime.Substring(0, 1);
                    Month = strDateTime.Substring(2, 3);
                    Year = strDateTime.Substring(6, 4);
                    Hour = strDateTime.Substring(11, 2);
                    Minutes = strDateTime.Substring(14, 2);
                    seconds = strDateTime.Substring(17, 2);
                }
                //Console.WriteLine(Day + '/' + Month + '/' + Year + ' ' + Hour + ':' + Minutes + ':' + seconds);
                sDateTime = Day + '/' + Month + '/' + Year + ' ' + Hour + ':' + Minutes + ':' + seconds;
